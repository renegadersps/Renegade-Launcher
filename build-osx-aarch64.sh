...
APPBASE="build/macos-aarch64/Renegade.app"
...
build() {
    ...
    cp native/build-aarch64/src/Renegade $APPBASE/Contents/MacOS/
    cp target/Renegade.jar $APPBASE/Contents/Resources/
    ...
    echo Setting world execute permissions on Renegade
    pushd $APPBASE
    chmod g+x,o+x Contents/MacOS/Renegade
    popd

    otool -l $APPBASE/Contents/MacOS/Renegade
}
...
dmg() {
    ...
    mv Renegade\ *.dmg Renegade-aarch64.dmg

    # dump for CI
    hdiutil imageinfo Renegade-aarch64.dmg

    if ! hdiutil imageinfo Renegade-aarch64.dmg | grep -q "Format: ULFO" ; then
        echo Format of dmg is not ULFO
        exit 1
    fi

    if ! hdiutil imageinfo Renegade-aarch64.dmg | grep -q "Apple_HFS" ; then
        echo Filesystem of dmg is not Apple_HFS
        exit 1
    fi

    # Notarize app
    if xcrun notarytool submit Renegade-aarch64.dmg --wait --keychain-profile "AC_PASSWORD" ; then
        xcrun stapler staple Renegade-aarch64.dmg
    fi
}
